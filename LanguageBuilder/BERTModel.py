...
class WordTrainer(object):
    def __init__(self, dir_name="textfiles",):
    ...
    def openfile(self, file_name):
        ...
        sentences=filter(lambda sentence:len(sentence.split())>4,self.files[file_name])
...
def main():
    ...
    LanguageDB=Connect(URL)
    print("connected to database")
    Sentences=LanguageDB["Sentences"]
    Sentences.insert_many(list([sentence for sentence in samples]))
    print("Languages found :" + list(samples.knownlanguages))
    WriteLanguages(list(samples.knownlanguages))
